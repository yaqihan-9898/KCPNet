...
from lib.nets.densenet import densenet
...
class Train:
    def __init__(self):
        ...
        elif cfg.FLAGS.network.startswith('densenet'):
            self.net = densenet(batch_size=cfg.FLAGS.ims_per_batch)
        elif cfg.FLAGS.network.startswith('resnet'):
    ...
    def train(self):
        ...
        fi = open(cfg.FLAGS.summary_path+'/loss.txt','w')
        fi.close()
        ...
        while iter < cfg.FLAGS.max_iters + 1:
            ...
            if iter % cfg.FLAGS.summary_per_iter !=0:
                try:
                    rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, attention_loss, hcf_loss, total_loss = self.net.train_step(
                        sess, blobs, iter,
                        train_op)
                except Exception:
                ...
                try:
                    rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, attention_loss, hcf_loss, total_loss, smy = self.net.train_step_with_summary(
                        sess, blobs, iter,
                        train_op)
            ...
            if iter % (cfg.FLAGS.display) == 0:
                with open(
                        cfg.FLAGS.summary_path+'/loss.txt',
                        'a') as f:
                    f.write('iter: %d / %d, total loss: %.6f\n >>> rpn_loss_cls: %.6f\n '
                            '>>> rpn_loss_box: %.6f\n >>> loss_cls: %.6f\n >>> attention_loss: %.6f\n >>> loss_box: %.6f\n >>> hcf_loss: %.6f\n ' % \
                            (iter, cfg.FLAGS.max_iters, total_loss, rpn_loss_cls, rpn_loss_box, loss_cls,
                            attention_loss, loss_box, hcf_loss))
