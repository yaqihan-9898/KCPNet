...
def anchor_target_layer(rpn_cls_score, gt_boxes, im_info, _feat_stride, all_anchors, num_anchors):
    ...
    '''
    anchor_target_layer主要针对RPN的输出进行处理，对RPN的输出结果加工，对anchor打上标签，
    ...
    if not cfg.FLAGS.rpn_clobber_positives:  # 将所有满足负样本label阙值的anchor标记为0
        # assign bg labels first so that positive labels can clobber them
        # first set the negatives
        labels[max_overlaps < cfg.FLAGS.rpn_negative_overlap] = 0

    # fg label: for each gt, anchor with highest overlap
    # 前景标签，对于每一个gt_boxes,和anchors重合率最大的检测框标记为1
    ...
    if len(fg_inds) > num_fg:  # 如果超过了制定的数量，使用random.choice()随机采样
        ...
        labels[disable_inds] = -1  # 将随机出来额外的标记为don't care
